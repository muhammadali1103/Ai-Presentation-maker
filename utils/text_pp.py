...
def parse_response(response, text_limit_per_slide=200):
    slides = response.split('\n\n')
    slides_content = []
    ...
    for slide in slides:
        ...
        # Check if content exceeds the text limit per slide
        if len(content.split()) > text_limit_per_slide:
            content = " ".join(content.split()[:text_limit_per_slide])
    ...
    if template_choice == 'dark_modern':
        for placeholder in slide.placeholders:
            if placeholder.placeholder_format.type == 1:  # Title
                ...
                placeholder.text = "This Presentation is downloaded From PresentationXpert"
                for paragraph in placeholder.text_frame.paragraphs:
                    for run in paragraph.runs:
                        run.font.name = 'Times New Roman'
