...
@login_required
def edit_profile():
    form = EditProfileForm()  # Create an instance of the EditProfileForm
...
@login_required
def generate():
    form = GenerateForm(request.form)  # Initialize the form with request data
    ...
    if request.method == 'POST':
        ...
        # Get the text_limit_per_slide value and provide a default of 200 if it's empty
        text_limit_per_slide = int(request.form['text_limit_per_slide']) if request.form.get('text_limit_per_slide') else 200
        ...
        slides_content = parse_response(assistant_response, text_limit_per_slide)

        create_ppt(slides_content, template_choice, presentation_title, presenter_name, insert_image)

    return render_template('generator.html', title='Generate', form=form)
